...
namespace MintCyclingService.UserAccount
{
    public class UserAccountService : IUserAccountService
    {
        ...
        /// <returns></returns>
        public ResultModel AddDeposit(AddDeposit_PM model)
        {
            var result = new ResultModel();
            using (var db = new MintBicycleDataContext())
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        UserDepositRecharge.UserInfoGuid = model.UserInfoGuid;
                        UserDepositRecharge.Amount = model.Amount;
                        //充值的方式：1支付宝；2微信
                        UserDepositRecharge.RechargeType = model.RechargeType == 1 ? "支付宝" : "微信";
                        //充值的状态：0充值失败；1充值成功
                        UserDepositRecharge.Status = model.Status;
                        UserDepositRecharge.PayDate = DateTime.Now;
                        UserDepositRecharge.OutTradeNo = model.Out_trade_no;           //订单号码
                        UserDepositRecharge.Trade_no = model.Trade_no;               //支付宝交易号
                        UserDepositRecharge.Sign_type = model.Sign_type;            // 签名类型RSA2
                        UserDepositRecharge.Sign = model.Sign;                      // 签名
                        UserDepositRecharge.Remark = " 用户Guid为" + model.UserInfoGuid + "充值押金为：" + model.Amount + "元。";
                        UserDepositRecharge.CreateBy = model.UserInfoGuid;
            ...
            using (var db = new MintBicycleDataContext())
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        UserDepositRecharge.Sign_type = model.Sign_type;            // 签名类型RSA2
                        UserDepositRecharge.Sign = model.Sign;                      // 签名
                        UserDepositRecharge.Remark = " 用户Guid为" + model.UserInfoGuid + "退押金了99元。";
                        UserDepositRecharge.CreateBy = model.UserInfoGuid;
                        UserDepositRecharge.CreateTime = DateTime.Now;
                        UserDepositRecharge.IsDeleted = false;
